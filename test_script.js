...
function navTo(id) {
  ...
  if(id==='home-screen' || id==='program-screen') {
    document.getElementById('nav-home').classList.add('active');
  }
  if(id==='home-screen') {
    renderProgramsList();
  }
  if(id==='program-screen') {
    renderProgram();
  }
...
function renderProgramsList() {
  ...
  userPrograms.forEach(function(prog) {
    ...
    var totalW = 0, compW = 0;
    prog.months.forEach(function(m){
      m.workouts.forEach(function(w){
        totalW++;
        var d = localStorage.getItem('ws_' + prog.instanceId + '_' + w.id + '_w' + (m.weeks.split('–')[0]||1));
        if (d) { try { var j=JSON.parse(d); if(j.s==='completed') compW++; } catch(e){} }
      });
    });
